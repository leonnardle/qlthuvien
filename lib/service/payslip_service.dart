      ...
      final future = list.map((loanslip) async {
        try {
          final bookresponse = await http.get(Uri.parse(
              '${ConFig.apiUrl}/phieutra/${loanslip.id}/danhsachsachtra'));
...
Future<bool> insertPaySlip(PaySlip paySlip) async {
  try {
    final response = await http.post(
      Uri.parse('${ConFig.apiUrl}/phieutra/'),
      headers: {
    ...
    if (response.statusCode == 200) {
      ...
      print(
          'Đã xảy ra lỗi khi thêm phieutra. Mã lỗi: ${response.statusCode}, Nội dung: ${response.body}');
      return false;
    }
  } catch (e) {
